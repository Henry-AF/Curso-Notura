// forked from design-sync lib/source-kit.mjs — synth-entry mode used
// `export * from <file>` only, which ES modules never re-export through for
// default exports (ShinyText/TextType use `export default`), so those
// components never reached window.<globalName>. Adds a default-as-<Base>
// re-export per file when the file actually has a default export.
//
...
export async function resolvePackage(ctx) {
  ...
  if (!entry) {
    ...
    const entryLines = comps.map((p) => `export * from ${JSON.stringify(p)};`);
    // `export *` never carries default exports — add an explicit named
    // re-export for files that have one, aliased to the file's own base
    // name (the same convention the src-matching heuristic below assumes).
    for (const p of comps) {
      const base = basename(p).replace(/\.(tsx|jsx)$/, '');
      if (/^[A-Z][A-Za-z0-9]*$/.test(base) && /export\s+default\b/.test(readText(p))) {
        entryLines.push(`export { default as ${base} } from ${JSON.stringify(p)};`);
      }
    }
